...
import { FetchHttpTextClient, KagiSearchProvider } from "./providers/kagi.ts";
		...
		parameters: Type.Object({
			...
			depth: Type.Optional(
				StringEnum([...SEARCH_DEPTHS], {
					description:
						"Search depth hint. Accepted for compatibility; the current Kagi provider ignores depth.",
				}),
			),
		}),
...
function createSearchProvider(settings: WebToolsSettings["search"]): SearchProvider {
	switch (settings.provider) {
		case "kagi":
			return new KagiSearchProvider(settings.endpoint, new FetchHttpTextClient());
	}
}
